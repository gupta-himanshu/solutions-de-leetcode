...
class Solution:
    def nodesBetweenCriticalPoints(self, head: Optional[ListNode]) -> List[int]:
        if not head or not head.next or not head.next.next:
            ...
            while nxt:
                is_local_maxima = prev.val < curr.val and nxt.val < curr.val
                is_local_minima = prev.val > curr.val and nxt.val > curr.val

                if is_local_maxima or is_local_minima:
                    if first_critical_pt == -1:
                        first_critical_pt = pos
                    else:
                        min_dist = min(min_dist, pos - last_critical_pt)
                    last_critical_pt = pos
            ...
            if first_critical_pt == last_critical_pt or first_critical_pt == -1:
                return [-1, -1]
            else:
                return [min_dist, last_critical_pt - first_critical_pt]
