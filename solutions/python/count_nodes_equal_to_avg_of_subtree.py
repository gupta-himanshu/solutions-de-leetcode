...
class Solution:
    def averageOfSubtree(self, root: TreeNode) -> int:
        ...
        def avgOfNodesSubtree(node: TreeNode) -> tuple[int, int]:
            nonlocal matching_nodes
            if not node:
                return 0, 0

            left_sum, left_count = avgOfNodesSubtree(node.left)
            right_sum, right_count = avgOfNodesSubtree(node.right)
