 ...
 * while 15 does not. Therefore 15 is the smallest missing integer greater than or equal to the sum of the longest
 * sequential prefix.
 *
 *
 * Constraints:
...
object Solution {
  def missingInteger(nums: Array[Int]): Int = {
    val longestSeqPrefix =
      nums.tail
        .zip(nums)
        .takeWhile { case (curr, prev) => curr == prev + 1 }
        .map(_._1)
        .prepended(nums.head)

    val start = longestSeqPrefix.sum

    LazyList.iterate(start)(_ + 1)
      .find(!nums.contains(_))
      .   get
  }
}
