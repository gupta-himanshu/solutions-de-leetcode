 ...
 * Input: digits = [0,2,2]
 * Output: 2
 * Explanation: The only 3-digit even numbers that can be formed are 202 and 220. Note that the digit 2 can be used twice
...
object Solution {
  def totalNumbers(digits: Array[Int]): Int = {
    ...
    (0 until n).flatMap(i => {
      (0 until n).flatMap(j => {
        (0 until n).map(k => {
          (((digits(i) * 100) + (digits(j) * 10) + digits(k)), (i, j, k))
        })
      })
    }).filter((num, idxs) => num > 99 && num % 2 == 0 && idxs._1 != idxs._2 && idxs._2 != idxs._3 && idxs._1 != idxs._3).map(_._1).toSet.size
  }
}
