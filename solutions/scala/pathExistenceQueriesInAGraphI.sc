 ...
 * You are also given an integer array nums of length n sorted in non-decreasing (ascending) order, and an integer maxDiff.
  ...
  def pathExistenceQueries(n: Int, nums: Array[Int], maxDiff: Int, queries: Array[Array[Int]]): Array[Boolean] = {
    val group =
      nums.tail
        ...
        .scanLeft(0) {
          case (g, (curr, prev)) =>
            if (curr - prev > maxDiff) g + 1
            else g
        }

    queries.map(q => group(q(0)) == group(q(1)))
  }
}
