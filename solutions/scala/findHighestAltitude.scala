...
object Solution {
    def largestAltitude(gain: Array[Int]): Int = {
        gain.toList.foldLeft((0, 0)){ case((currAltitude, maxAltitude), g) =>
            val newCurrAltitude = currAltitude + g
            val newMaxAltitude = math.max(newCurrAltitude, maxAltitude)
            (newCurrAltitude, newMaxAltitude)
        }._2
    }
}