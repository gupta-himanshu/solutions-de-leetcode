 ...
 *
 * You are given two integers n and t. Return the smallest number greater than or equal to n such that the product of its
...
object Solution {
  def smallestNumber(n: Int, t: Int): Int = {
    @tailrec
    def product(i: Int, acc: Int): Int = {
      if (i > 0) {
        product(i / 10, acc * (i % 10))
      } else {
        acc
      }
    }

    (n to 100).find(i => product(i, 1) % t == 0).get
  }
}
