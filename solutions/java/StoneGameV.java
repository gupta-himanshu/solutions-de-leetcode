...
class Solution {
    ...
    // Recursive DP: returns max score from subarray [l, r]
    private int dfs(int l, int r) {
        if (l == r) return 0; // Only one stone, no score possible
        if (memo[l][r] != null) return memo[l][r];

        int best = 0;

        // Try splitting between l..i and i+1..r
        for (int i = l; i < r; i++) {
            int leftSum = rangeSum(l, i);
            int rightSum = rangeSum(i + 1, r);

            if (leftSum < rightSum) {
                best = Math.max(best, leftSum + dfs(l, i));
            } else if (leftSum > rightSum) {
                best = Math.max(best, rightSum + dfs(i + 1, r));
            } else { // Equal sums
                best = Math.max(best, leftSum + Math.max(dfs(l, i), dfs(i + 1, r)));
            }
        }
    ...
    // Helper to get sum of stones in range [l, r]
